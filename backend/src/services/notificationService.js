...
// Instancia Resend uma única vez (singleton)
const resend = process.env.RESEND_API_KEY ? new Resend(process.env.RESEND_API_KEY) : null;

/**
 * Serviço de notificações
 * Email via Resend (MVP) | preparado para WhatsApp
 */

async function sendEmailAlert({ user, search, priceData, analysis, message }) {
  const { price, airline, stops, departure_datetime, arrival_datetime, deep_link } = priceData;
  const { opportunity } = analysis;

  const formattedPrice = price.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' });
  const stopsText = stops === 0 ? 'Voo direto' : `${stops} escala${stops > 1 ? 's' : ''}`;
  const departureDate = new Date(departure_datetime).toLocaleDateString('pt-BR', {
    weekday: 'short', day: 'numeric', month: 'short', hour: '2-digit', minute: '2-digit'
  });

  const html = buildEmailHTML({
    user, search, formattedPrice, airline, stopsText, departureDate,
    message, deep_link, opportunity, searchId: search.id
  });

  if (!resend) {
    // Em dev, apenas loga (sem API key real)
  ...
  try {
    ...
    console.error(`📧 Falha ao enviar email para ${user.email}:`, err.message);
    return { success: false, error: err.message };
  }
}

function buildEmailHTML({ user, search, formattedPrice, airline, stopsText, departureDate, message, deep_link, opportunity, searchId }) {
  // Gera links funcionais de gestão (token simples base64)
  const token = Buffer.from(`${user.id || 'unknown'}:${searchId || ''}`).toString('base64');
  const appUrl = process.env.APP_URL || process.env.FRONTEND_URL || 'http://localhost:3000';
  const unsubscribeUrl = `${appUrl}/unsubscribe?token=${token}`;
  const manageUrl = `${appUrl}/dashboard`;

  const severityColor = {
    high: '#ef4444',
    medium: '#f97316',
    low: '#3b82f6',
  }[opportunity?.severity] || '#3b82f6';
    ...
    <tr><td align="center" style="padding:32px 16px">
      <table width="600" cellpadding="0" cellspacing="0" style="max-width:600px;background:#fff;border-radius:16px;overflow:hidden;box-shadow:0 4px 24px rgba(0,0,0,0.08)">
        ...
        <tr><td style="padding:0 32px">
          <div style="margin:-1px 0;background:${severityColor};padding:10px 20px;text-align:center">
            <span style="color:#fff;font-size:13px;font-weight:600;text-transform:uppercase;letter-spacing:0.5px">
              ${opportunity?.label || 'Oportunidade detectada'}
            </span>
          </div>
        </td></tr>
        ...
        <tr><td style="padding:32px">
          <table width="100%" cellpadding="0" cellspacing="0">
            <tr>
              ...
              <td width="16"></td>
              <td style="text-align:center;padding:16px;background:#f8fafc;border-radius:12px">
                <p style="color:#64748b;font-size:12px;margin:0 0 4px;text-transform:uppercase">Escalas</p>
          ...
          <div style="text-align:center;margin-top:24px">
            <a href="${deep_link}" style="display:inline-block;background:#3b82f6;color:#fff;padding:16px 40px;border-radius:10px;font-size:16px;font-weight:700;text-decoration:none">
              Ver Passagem Agora →
            </a>
          </div>
        </td></tr>
        ...
        <tr><td style="padding:24px 32px;border-top:1px solid #e2e8f0;text-align:center">
          <p style="color:#94a3b8;font-size:12px;margin:0">
            Você recebeu este alerta porque configurou um monitoramento.<br>
            <a href="${manageUrl}" style="color:#94a3b8">Gerenciar alertas</a> · 
 ...
 */
function recordAlertSent({ searchId, userId, priceHistoryId, alertType, triggerValue, message, channel = 'email' }) {
  const id = uuidv4();
  db.prepare(`
    INSERT INTO alerts_sent (id, search_id, user_id, price_history_id, alert_type, trigger_value, message, channel)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?)
  `).run(id, searchId, userId, priceHistoryId, alertType, triggerValue, message, channel);
  return id;
}
